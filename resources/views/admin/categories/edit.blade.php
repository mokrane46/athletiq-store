...
<div class="premium-page-header" style="text-align: left; margin-bottom: 50px;">
    ...
    <h1 class="premium-title">Edit Category</h1>
</div>
<div class="premium-card" style="max-width: 550px;">
    <form action="{{ route('admin.categories.update', $category) }}" method="POST">
        ...
        <div class="premium-form-group" style="margin-bottom: 40px;">
            ...
            <input type="text" name="Category_name" id="Category_name" class="premium-input" placeholder="e.g. Footwear" value="{{ old('Category_name', $category->Category_name) }}" required autofocus>
            <p style="font-size: 0.75rem; color: var(--text-muted); margin-top: 10px;">This will update the department name across the entire storefront.</p>
            @error('Category_name')<p style="color: #ff3e3e; font-size: 0.8rem; margin-top: 8px;">{{ $message }}</p>@enderror
        </div>
        <div style="display: flex; gap: 20px;">
            <button type="submit" class="premium-btn" style="flex: 1.5;">Save Changes</button>
