...
<div class="premium-page-header" style="text-align: left; margin-bottom: 50px;">
    ...
    <h1 class="premium-title">Edit Product</h1>
</div>
    ...
    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px; margin-bottom: 30px;">
            <div class="premium-form-group" style="grid-column: span 2;">
                ...
                <input type="text" name="Product_name" id="Product_name" class="premium-input" placeholder="e.g. Performance Tech Tee" value="{{ old('Product_name', $product->Product_name) }}" required>
            ...
            <div class="premium-form-group">
                ...
                <input type="number" step="0.01" name="Price" id="Price" class="premium-input" placeholder="0.00" value="{{ old('Price', $product->Price) }}" required>
                @error('Price')<p style="color: #ff3e3e; font-size: 0.8rem; margin-top: 8px;">{{ $message }}</p>@enderror
            </div>

            <div class="premium-form-group">
                <label class="premium-label" for="Quantity">Stock Quantity</label>
                <input type="number" name="Quantity" id="Quantity" class="premium-input" placeholder="0" value="{{ old('Quantity', $product->Quantity) }}" required>
                ...
                <select name="SubCategory_ID" id="SubCategory_ID" class="premium-input" required>
                    ...
                    @foreach($subcategories as $sub)
                        ...
                        <option value="{{ $sub->SubCategory_ID }}" 
                                ...
                                {{ old('SubCategory_ID', $product->SubCategory_ID) == $sub->SubCategory_ID ? 'selected' : '' }}>
        ...
        <div class="premium-form-group" style="margin-bottom: 40px;">
            ...
            <div style="display: flex; gap: 25px; align-items: flex-start;">
                @if($product->Product_image)
                    <div style="width: 120px; height: 120px; border-radius: 12px; overflow: hidden; border: 1px solid rgba(0,0,0,0.05); box-shadow: 0 5px 15px rgba(0,0,0,0.05);">
                        @php
                            $imagePath = 'images/' . $product->Product_image;
                            if (!file_exists(public_path($imagePath))) {
                                $imagePath = 'storage/products/' . $product->Product_image;
                            }
                        @endphp
                        <img src="{{ asset($imagePath) }}" alt="Current" style="width: 100%; height: 100%; object-fit: cover;">
                    </div>
                @endif
                <div style="flex: 1; border: 2px dashed rgba(50, 255, 126, 0.2); border-radius: 15px; padding: 30px; text-align: center; position: relative; transition: all 0.3s ease; background: rgba(50, 255, 126, 0.02);" onmouseover="this.style.borderColor='var(--accent)'; this.style.background='rgba(50, 255, 126, 0.05)'" onmouseout="this.style.borderColor='rgba(50, 255, 126, 0.2)'; this.style.background='rgba(50, 255, 126, 0.02)'">
                    <i class='bx bx-cloud-upload' style="font-size: 2.5rem; color: var(--accent); margin-bottom: 10px; display: block;"></i>
                    <p style="font-size: 0.85rem; color: var(--text-secondary); font-weight: 600;">Drag & Drop or Click to Upload</p>
                    <p style="font-size: 0.7rem; color: var(--text-muted); margin-top: 5px;">SVG, PNG, JPG or GIF (max. 2MB)</p>
                    <input type="file" name="Product_image" id="Product_image" style="position: absolute; top:0; left:0; width:100%; height:100%; opacity:0; cursor:pointer;">
                    <div id="file-name-display" style="font-size: 0.8rem; font-weight: 700; color: var(--accent); margin-top: 10px; display: none;"></div>
                </div>
            </div>
        ...
        <div style="display: flex; gap: 20px;">
            <button type="submit" class="premium-btn" style="flex: 2;">Save Product Updates</button>
            <a href="{{ route('admin.products.index') }}" class="premium-btn premium-btn-outline" style="flex: 1; text-decoration: none;">Discard Changes</a>
        </div>
    </form>
</div>

@php
    $currentSpecs = $product->specifications->mapWithKeys(function($spec) {
        return [$spec->Spec_name => $spec->pivot->Spec_value];
    })->toArray();
@endphp
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const currentSpecs = @json($currentSpecs);
    ...
    const updateFields = () => {
        ...
        if (category === 'clothes') {
            ...
            fields.forEach(f => {
                ...
                const curVal = currentSpecs[f.name] || '';
                
                if (f.type === 'select') {
                    let optionsHTML = `<option value="">Select ${f.label}</option>` +
                                      f.options.map(o => `<option value="${o}" ${curVal === o ? 'selected' : ''}>${o}</option>`).join('');
                    ...
                    div.innerHTML = `<label class="premium-label">${f.label}</label>
                                     <input type="text" name="specifications[${f.name}]" class="premium-input" placeholder="Enter ${f.label}" value="${curVal}" required>`;
                }
                dynamicFields.appendChild(div);
            });
        } else if (category === 'weights' || category === 'dumbbells') {
            hasFields = true;
            const curWeight = currentSpecs['Weight'] || '';
            const div = document.createElement('div');
            div.className = 'premium-form-group';
            div.style.gridColumn = 'span 2';
            div.innerHTML = `<label class="premium-label">Weight Specification (kg)</label>
                             <input type="number" step="0.1" name="specifications[Weight]" class="premium-input" placeholder="e.g. 10.5" value="${curWeight}" required>`;
            dynamicFields.appendChild(div);
        }
